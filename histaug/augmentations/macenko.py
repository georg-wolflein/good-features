...
from torchstain.torch.utils import cov, percentile as _percentile
...
class FullyTransparentException(Exception):
    pass


def percentile(t, q):
    if t.numel() == 0:
        print("Warning: percentile called on empty tensor")
    return _percentile(t, q)


class TorchMacenkoNormalizer(HENormalizer, nn.Module):
    def __init__(self):
        super().__init__()

        self.register_buffer("HERef", torch.tensor([[0.5626, 0.2159], [0.7201, 0.8012], [0.4062, 0.5581]]))
        self.register_buffer("maxCRef", torch.tensor([1.9705, 1.0308]))
    ...
    def normalize(self, I, Io=240, alpha=1, beta=0.15, stains=True):
        ...
        Inorm[Inorm > 255] = 255
        Inorm = Inorm.reshape(c, h, w).type_as(I)
